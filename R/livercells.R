#'livercells is A package for deconvolute the liver RNA-seq data for cell composition
#'
...
livercells <- function(expr, signatures=NULL, genes= NULL, parallel.sz = 4, parallel.type = 'SOCK') {
  ...
  if (is.null(signatures))
    signatures = liver_data$signatures
  if (is.null(genes))
    genes = liver_data$genes
